...
function ComparisonTable({ pools }: { pools: Pool[] }) {
  const metrics = [
    ...
    { label: 'TVL', key: 'totalStaked', format: (val: any) => `$${(val / 1000000).toFixed(2)}M` },
    { label: 'Available Liquidity', key: 'availableLiquidity', format: (val: any) => `$${(val / 1000).toFixed(0)}K` },
    { label: 'Total Investors', key: 'totalInvestors', format: (val: any) => val.toLocaleString() },
    { label: 'Minimum Stake', key: 'minimumStake', format: (val: any) => `$${val.toLocaleString()}` },
