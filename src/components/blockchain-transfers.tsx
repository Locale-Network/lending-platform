...
export function BlockchainTransfers() {
  const user = useUser();
  const { transfers, loading, error, hasMore, loadMore, refresh } = useAlchemyTransfers('external');
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>Connect your wallet to view on-chain transactions</CardDescription>
        </CardHeader>
        <CardContent>
  ...
  if (error) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>Blockchain Transactions</CardTitle>
          <CardDescription>Verified on-chain transaction history</CardDescription>
        </CardHeader>
        <CardContent>
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            <CardTitle>Blockchain Transactions</CardTitle>
            <CardDescription>Verified on-chain transaction history from Alchemy</CardDescription>
          </div>
          <Button
            variant="outline"
            size="sm"
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className="flex h-10 w-10 items-center justify-center rounded-full bg-muted">
                      {transfer.from.toLowerCase() === user.address?.toLowerCase() ? (
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="flex items-center gap-2 text-sm text-muted-foreground">
                        ...
                        <span>•</span>
                        <a
                          href={`https://arbiscan.io/tx/${transfer.hash}`}
                          target="_blank"
